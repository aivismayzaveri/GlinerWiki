...
Add a document or directory of documents. Always re-processes on explicit add (hash-based skip only applies in `openkb watch` mode).
...
1. Hash removal — if file is already known, remove hash to force reprocess (explicit add always reprocesses)
